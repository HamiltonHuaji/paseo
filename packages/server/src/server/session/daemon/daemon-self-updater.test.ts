...
describe("DaemonSelfUpdater", () => {
  ...
  test("updates a bundled fork using its outer distribution version", async () => {
    const calls: RuntimeCall[] = [];
    ...
    const updatedForkInstall = { ...forkInstall, version: "0.6.1-fork.2" };
    const runtime = createRuntime({
      calls,
    ...
    const { result } = await runUpdate({ runtime, daemonVersion: "0.6.1" });

    expect(result).toEqual({ success: true, error: null, newVersion: "0.6.1-fork.2" });
    expect(calls).toEqual(["inspect", "installLatest", "inspect"]);
  });
