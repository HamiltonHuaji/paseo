...
class PaseoPseudoterminal implements vscode.Pseudoterminal {
  ...
  private terminalSubscription: { release(): Promise<unknown> } | null = null;
  ...
  private async subscribe(): Promise<void> {
    try {
      ...
      if (this.closed) {
        await response.subscription.release();
        return;
      }
      this.terminalSubscription = response.subscription;
  ...
  private finish(exitCode?: number): void {
    ...
    this.closed = true;
    if (this.terminalSubscription) void this.terminalSubscription.release();
