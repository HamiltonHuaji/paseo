...
function parseHostPort(value: string): { host: string; port: string } | null {
  ...
  try {
    ...
    return { host: normalizeHost(parsed.hostname), port: parsed.port };
  } catch {
    return null;
  }
}
...
export function parseDaemonEndpointCandidate(
  ...
  if (!parsed) {
    return { source: candidate.source, message: `Invalid daemon endpoint: ${value}` };
  }
  return { source: candidate.source, endpoint: `${parsed.host}:${parsed.port}` };
}
...
function getStatusInfo(raw: unknown): DaemonStatusInfo {
  ...
  return { serverId: toNonEmptyString(raw.serverId), version: toNonEmptyString(raw.version) };
}
...
export async function probeDaemonStatus(input: {
  ...
  try {
    const response = await (input.fetch ?? defaultFetch)(
    ...
    return { status: "ok", ...getStatusInfo(await response.json()) };
  } catch {
    return { status: "unreachable", serverId: null, version: null };
  }
}
...
export async function discoverDaemonEndpoint(
  ...
  return {
    endpoint: firstParsed?.endpoint ?? DEFAULT_ENDPOINT,
    source: firstParsed?.source ?? "default",
    requiresPassword: false,
    available: false,
    serverId: null,
    version: null,
    unsupportedMessage: firstUnsupported?.message ?? null,
  };
}
