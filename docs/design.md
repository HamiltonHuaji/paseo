...
Desktop workspace tabs retain a short readable label before overflowing horizontally; they do not collapse to icon-only tabs to avoid overflow. The tab strip consumes both horizontal side-wheel input and ordinary vertical wheel input while it can scroll, and activating an off-screen tab reveals it without moving an already visible tab.

Web and Electron users may prefer workspace tabs on the left. This is a pane-local responsive preference: compact layouts keep the tab switcher, non-web clients keep top tabs, and a web pane uses the 220px rail only when at least 420px remains for pane content (640px total). Narrow split panes therefore fall back to top tabs independently. The rail scrolls vertically, keeps creation/pinned/split actions in a fixed footer, and uses above/below semantics for ordering actions.
