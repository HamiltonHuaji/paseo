...
Codex `request_user_input_async` emits an `agentMessage` with `delivery: "async"` and structured `questions`. Keep those questions on the assistant timeline item; its text is only the transcript form of the same prompt. The answer is a new user message linked to that assistant message ID, and can steer an active turn. The blocking `item/tool/requestUserInput` request remains a question permission with a server-request response. Do not move asynchronous questions into the permission lifecycle.
