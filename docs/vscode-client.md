...
The `Fork VS Code Release` workflow is opt-in and builds the same stable asset. With
`publish=true`, it attaches the VSIX to an existing fork release tag; with `publish=false`, it keeps
a seven-day workflow artifact. The extension version uses the canonical fork version from
`fork-build-info.json`.
