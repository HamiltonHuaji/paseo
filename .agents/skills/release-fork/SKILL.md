...
for derivation, and use `A.B.(C * 1000 + R)`. For example, upstream `0.9.0-beta.2` revision `1` is
`0.9.1`. `fork-build-info.json` stores the derived version, exact upstream baseline, and revision; reject a
