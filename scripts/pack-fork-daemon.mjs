...
export function buildForkDaemonPackageManifest({
  ...
  return {
    ...
    repository: { type: "git", url: "https://github.com/HamiltonHuaji/paseo.git" },
  };
}
...
export function buildDistributionManifest(packageManifest, internalManifests) {
  ...
  if (!serverManifest) throw new Error("The fork daemon package requires @getpaseo/server");
  return {
    schemaVersion: 1,
...
async function listPackedFiles(packageDirectory) {
  ...
  if (!result?.files) throw new Error(`npm pack did not report files for ${packageDirectory}`);
...
async function copyCliPayload() {
  ...
  for (const relativePath of await listPackedFiles(cliDirectory)) {
    if (relativePath === "package.json" || relativePath === "bin/paseo") continue;
    const targetPath = path.join(STAGE_DIRECTORY, relativePath);
    await mkdir(path.dirname(targetPath), { recursive: true });
    await copyFile(path.join(cliDirectory, relativePath), targetPath);
  }
}
...
async function packStage() {
  ...
  return { path: OUTPUT_TARBALL, size: result.size, bundled: [...bundled].sort() };
}
